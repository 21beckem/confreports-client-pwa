...
const formData = {
    ...
    allowVariations : urlPrms.get('allowVariations')
}
const formData_formated = {
    ...
    phraseToSearch : urlPrms.get('phraseToSearch'),
    allowVariations : (urlPrms.get('allowVariations') == 'true')
}
...
async function main() {
    ...
    for (let i = 0; i < flatList.length; i++) {
        ...
        const confData = fetch('https://21beckem.github.io/conference-data/' + thisConfId + '-full.json').then(res => res.json()).then(res => searchConfrence(formData.phraseToSearch, res));
